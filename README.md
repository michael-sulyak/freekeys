...
### Установка
1. Скачайте архив и распакуйте в нужно место.
...
### Настройки
Все настройки содержаться в файле config.json.
...
###### ВКонтакте
auth_vk – разрешить или запретить авторизацию через ВКонтакте
auth_vk_app_id – id ВКонтакте приложения для авторизации
auth_vk_app_secret – секретный ключ приложения 

###### Facebook
auth_facebook - разрешить или запретить авторизацию через Facebook
auth_facebook_app_id
auth_facebook_app_secret

###### Предметы
items_reg_points – кол-во выдаваемых при регистрации
items_ref_limit – кол-во пользователей засчитываемых за 1 час
...
```php
__('Unknown error.');
_s('Account or IP banned until %s.', date('H:i:s, d.m.Y', $date));
```

Для редактирования *.po файлов можете использовать Poedit.
Есть простой скрипт для создания/обновления lang.po файлов плагина / шаблона.
...
Для выполнения интерфейса нужно указать interface[items][save_task][0][run]
...
Пользователь в данном случае следует Вашим укзаниям. Например, зайти на сайт мой_сайт.ру и ввести свой id
...
Большое спасибо тем, кто поддерживает проект.
